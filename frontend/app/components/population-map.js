	...
	renderSectionsTest() {
		...
		d3.json("../assets/secciones.json", (error, data) => {
			...
			this.get('sectionsLayer').selectAll("path")
				.data(topojson.feature(data, data.objects.secciones).features.filterBy('properties.state_code', 19).filterBy('properties.mun_code', 39))
				.enter().append("path")
